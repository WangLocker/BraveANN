...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        public:
            ...
            //为了内存映射声明的成员变量 
            xt::xarray_adaptor<xt::xbuffer_adaptor<T*, xt::no_ownership>> X;

            Dataset()
            : X(xt::xbuffer_adaptor<T*, xt::no_ownership>(nullptr, 0), std::vector<std::size_t>{0, 0}) {}
            ...
            void Initialize(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_, T* data_ = nullptr, bool shareOwnership_ = true)
            {
                ...
                //added
                std::vector<std::size_t> shapeX = {rows_, cols_};
                X = xt::xarray_adaptor<xt::xbuffer_adaptor<T*, xt::no_ownership>>(
                    xt::xbuffer_adaptor<T*, xt::no_ownership>(data, rows_ * cols_),
                    shapeX
                );
                // 下面这段代码仅仅只是为了验证数据内容在内存映射后是否仍然一样
                // for(int i=0;i<128;i++){
                //     std::cout<<X(2357,i)<<",  ";
                //     float temp=*(data+128*2357+i);
                //     std::cout<<temp<<",  ";
                // }
                // *(data+128*2357+0)=114.0f;
                // *(data+128*2357+1)=514.0f;
                // *(data+128*2357+2)=1919.0f;
                // *(data+128*2357+3)=810.0f;
                // std::cout<<"\n\n";
                // for(int i=0;i<128;i++){
                //     std::cout<<X(2357,i)<<",  ";
                //     float temp=*(data+128*2357+i);
