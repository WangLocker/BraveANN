...
#pragma omp parallel for num_threads(args._T) shared(data, indices) reduction(+:currDist)
        ...
        inline float KMeansPlusPlus(const Dataset<T>& data, std::vector<SizeType>& indices, SizeType first, SizeType last, KmeansArgs<T>& args, int maxIters, float& lambda) {
            ...
            for (int iter = 0; iter < maxIters; iter++) {
                ...
                currDist = KmeansAssign_normalKM_init<T,R>(data, indices, first, last, args, true, lambda);

                std::cout<<"KM++ LOSS: "<<currDist<<std::endl;        
                if (std::abs(currDist - prevDist) <1e-4) {break;}
                prevDist = currDist;
                if (currDist < minClusterDist) {
                    minClusterDist = currDist;
                    memcpy(args.newTCenters, args.centers, sizeof(T)*args._K*args._D);
...
#pragma omp parallel for num_threads(num_threads) shared(masked_distances, max_val, thread_starts, thread_task_counts) reduction(+:exp_sum)
        ...
        void FastRKM_Assign(
            ...
            for(int j=0;j<num_epoch;j++){
                ...
                // phi = xt::load_npy<float>("/home/wym/workdir/spann_exp/temp/datasets/siftsmall/phi_0.npy");
                // args.labels=xt::load_npy<int>("/home/wym/workdir/spann_exp/temp/datasets/siftsmall/labels_0.npy");
                // args.centroids=xt::load_npy<float>("/home/wym/workdir/spann_exp/temp/datasets/siftsmall/centroids_0.npy");
                ...
                auto batch = xt::view(data.X, xt::keep(batch_indices), xt::all());
                // std::cout<<batch<<"\n\n";
                // std::cout<<args.centroids<<"\n\n";
                xt::xarray<float> distances_batch_min;
                xt::xarray<int> labels_batch;
                auto time_f1 = std::chrono::high_resolution_clock::now();
                ...
                // std::cout<<distances_batch_min<<"\n\n";
                // std::cout<<labels_batch<<"\n\n";
                auto time_s1 = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double> sec = time_s1 - time_s0;
                time22+=sec.count();
                //-------------------second level---------------------------
                //-------------------third level---------------------------
                ...
                std::chrono::duration<double> thi = time_t1 - time_t0;
                time33+=thi.count();
                //-------------------third level---------------------------
                //-------------------fouth level---------------------------
                ...
                // std::cout<<phi_batch<<std::endl;    
                auto tilted1 = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double> seconds_til = tilted1 - tilted0;
                time_tilted+=seconds_til.count();
                //-------------------fouth level---------------------------

                //-------------------fifth level---------------------------
                auto time_fi0 = std::chrono::high_resolution_clock::now();
                ...
                auto time_fi1 = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double> fif = time_fi1 - time_fi0;
                time55+=fif.count();
                //-------------------fifth level---------------------------
            ...
            // std::cout<<"===total cost: "<<seconds1.count()<<"\n\n";
            // std::cout<<"===lv 1  cost: "<<time11<<"\n\n";
            // std::cout<<"===lv 2  cost: "<<time22<<"\n\n";
            // std::cout<<"===lv 3  cost: "<<time33<<"\n\n";
            // std::cout<<"===tilted cost: "<<time_tilted<<"\n\n";
            // std::cout<<"===lv 5  cost: "<<time55<<"\n\n";
            float dis=KmeansAssign_normalKM<T,R>(data,indices,first,last,args,true,lambda);
        ...
        float TryRClustering_new(const Dataset<T>& data,
            ...
            float lambda=0.;
            KMeansPlusPlus<T,R>(data,indices,first,last,args,100,lambda);
            ...
            if((last-first)>=32){
                ...
                phi = compute_tilted_sse_InEachCluster(data.X,args.centroids,args.labels,args._K,-0.01);
            }
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                args.ClearCounts();
                args.ClearDists(-MaxDist);
                FastRKM_Assign<T,R>(data,indices,first,last,args,phi,-0.01f,5,0.05f,min(0.05f,lambda));
